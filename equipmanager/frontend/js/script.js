...
async function salvarEquipamento(event) {
  ...
  if (!payload.nome || !payload.categoria || !payload.patrimonio || !payload.localizacao) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.mensagem || "Não foi possível salvar o equipamento.");
